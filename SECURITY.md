...
| **Reflowable EPUB content** | Publication HTML and JavaScript | EPUB content is HTML by definition and is rendered in a restricted context with no access to app state, credentials or the file system beyond the publication. **Network egress is not among what that context restricts today**: a publication's remote images, fonts and scripts load as they would in a browser, so a crafted book can reach a host the reader never configured. Whether to deny that by default is [ADR-0014](docs/decisions/0014-epub-webview-network-egress.md) — open. |
