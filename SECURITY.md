...
| **RAR and TAR parsing** | `libarchive`, a C library, on untrusted bytes | Chosen partly *because* it is the most audited RAR implementation in open source. Every format reader we do not use is compiled out, shrinking the reachable surface. Reads still arrive through the same bounds-checked `RandomAccessSource`. |
