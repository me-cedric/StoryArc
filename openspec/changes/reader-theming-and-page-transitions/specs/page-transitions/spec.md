...
#### Scenario: Curl on reflowable content
- **WHEN** a curl runs over reflowable text
- **THEN** the turning page is a faithful raster of the page it replaces, at the display's scale, so no text reflows or shifts during the turn
- **AND** interaction with the text — selection, link taps — resumes the moment the turn completes
