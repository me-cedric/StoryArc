...
| 14 | **iOS: "Start from the beginning" is a dead control in the list layout.** The trailing closure binds to `onRefused` under Swift's forward-scan rule, so `onRestart` keeps its `{}` default and `CoverList` has no restart handling at all — yet the menu renders the button whenever a publication has progress. A reader long-presses, taps it, and nothing happens: no confirmation, no clear, no message. The grid path is correct; Android is immune because its sheet gates the row on `onRestart != null`. **Nothing failed** — which is why a test asserting the button exists would not have caught it either. | `CoverList.swift:122`, `AddToShelfMenu.swift:58-73` |
| 15 | **Restart diverges on the publication page.** iOS offers it, Android omits it. Same reader, same book, two answers. The new `reading-progress` delta was written to be true on both — it forbids restart being the *primary* action and is silent on the overflow — so **either answer is spec-legal and the divergence is simply undecided.** | `DetailActions.swift:146`, `PublicationDetailScreen.kt:258` |
| 16 | **Android: an unnamed 14 dp target on Downloads.** The route walk, now inspecting each screen it reaches, reports `UNNAMED View at [53,2132][353,2169]` and `SMALL View 114.3x14.1dp ""` — a hit target a third of the 48 dp floor, carrying no name. Found the first time anything looked at that screen rather than at whichever screen was in front of the person running the scan. | `pnpm smoke:android:a11y` |
| 17 | **Android: the EPUB reader's WebView is unnamed.** Arguably expected, since the page's own contents are the accessibility tree — but nothing says so, and a reader arriving on it by swipe hears nothing about where they are. Worth a decision rather than a shrug. | `pnpm smoke:android:a11y` |
