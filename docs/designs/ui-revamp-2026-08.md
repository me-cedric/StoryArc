...
| --- | --- | --- | --- |
| 1 | Search is a fourth plain `Tab` in the capsule, leading to `LibraryView(surface: .search)` | Search is a fourth `ShortNavigationBar` destination, leading to `SearchScreen` with `AppBarWithSearch` as its top bar | **Rewritten. What stood here — a separated `Tab(role: .search)` against a `TopSearchBar` on Home and Library — is what both platforms shipped and what `navigation-shell` was rewritten to forbid**: the role morphs the tab into a field in place and leaves no screen to land on, and a field belonging to the shelf makes searching something done *to* a shelf. Material's own exception clause covers the destination, and the judgement it is conditioned on is ours: publications arrive from five kinds of place and no shelf shows all of them at once. The bar's own container, the narrowing control and where recent searches sit still diverge — rows 14 to 16. |
...
| 6 | Bulk undo is an **inset glass capsule**, sharing a `GlassEffectContainer` with the selection's actions so their edges morph | Bulk undo is a `Snackbar` | Snackbar is Material's undo affordance; iOS has no snackbar and should not grow one. **The iOS half said `safeAreaBar` until 2026-09-02, and that was the defect rather than the design**: a `safeAreaBar` holding a full-bleed `Rectangle()` is a slab, and the same slab was drawn twice — once for the selection and once, one screen over, for the undo. Both are capsules now. |
| 6b | Selection chrome **replaces the tab bar**: it is hidden for the mode, the count moves to the navigation title, *Done* takes the toolbar's trailing slot, and the actions float as a capsule where the tab bar was | Selection chrome is a **contextual `TopAppBar`**: close at the start, the count as title, actions and an overflow — and the navigation bar is untouched for the whole mode | Photos, Files and Mail all replace the bottom surface rather than stacking on it, which is what stops an app showing two bottom bars. Android never puts selection chrome at the foot, because that is the navigation bar's territory. **Neither shape was specified until after it shipped** — `named-failures-and-quieter-chrome` §3b and its `native-experience` delta now state it, and the owner had reported the old one twice. |
...
| 14 | The bar is a floating capsule inset from the edges | The bar spans the window edge to edge, no inset and no rounding | `ShortNavigationBar` exposes no `shape` parameter and `ShortNavigationBarDefaults` has no shape member — verified with `javap` over `material3.aar` at 1.5.0-alpha26. The capsule there is not discouraged, it is inexpressible. |
| 15 | Narrowing the search is a `.searchScopes` segmented bar | Narrowing is `FilterChip`s | Material retired the segmented button in the Expressive update and its named replacement is specified for two to five *fixed* views; our sources are an open, growing set. iOS's scope bar is current and idiomatic there. |
| 16 | Recent searches are on the search page, beside the suggestions | Recent searches are inside the **expanded** search bar, with the clear affordance | iOS's field has no expanded state of its own to put them in, so its page carries both. Android's bar does, and it is where a reader who has pressed to type is looking — which is also how the owner asked for it: *propositions on the search page; once you press to type, previous search results show up*. The page carries the suggestions and the scope; the expanded bar carries the recents and the results. |
