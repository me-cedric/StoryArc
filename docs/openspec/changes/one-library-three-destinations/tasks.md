...
- [ ] **6.5** The screenshot set is complete: every task above that changes a
      ...
      *Re-counted 2026-09-05. **The list of six is a list of nineteen**, because 0b landed
      ...
      | 9 | An Android sectioned shelf (4); and either platform on record at **200+** publications | 3.4 | **Half done, and the other half is a defect report.** A 220-publication Android shelf is photographed in `docs/designs/screenshots/long-shelf-2026-09-11/`, five frames, grid and list, light and dark — so the *200+ on record* half is answered. **The shelf is not sectioned**, in either layout, with `THRESHOLD` at 12. `LibrarySections.divide` is correct and covered by eight passing cases, so the gap is in the wiring between it and the drawn shelf, and reading the source has not said which part. The README carries the evidence, the recipe and the next step. |
...
## Reversed on 2026-09-07: home shows the library's empty state
