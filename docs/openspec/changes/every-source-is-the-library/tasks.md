...
- [x] 3b.6 iOS now has all of it: the rule, both contributors, `ServerLibrary`, `LibraryMerge`, a series cell in `CoverGrid`, and `SeriesShelfView` reached by `SeriesRoute` — registered beside the publication page, so a stack that has one has both. **Partly seen on a simulator**: the app builds for iOS 26.2 and launches on an iPhone 17 Pro with these changes in it, and Home draws its empty state rather than crashing. The series shape itself is unproven there, because the simulator has no source configured and adding the owner's Kavita server needs their API key typed into the app.
