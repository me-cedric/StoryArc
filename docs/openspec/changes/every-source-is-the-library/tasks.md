...
- [ ] 3b.6 **Half done.** iOS has the rule — `LibraryRows` and `LibraryRowsTests`, the same seven cases as Android — and both contributors with their tests. It does **not** yet draw a series cell or have a screen to open one, so nothing an iOS reader sees has changed.
