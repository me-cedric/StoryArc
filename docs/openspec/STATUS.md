...
| `network-share` | partial | Reconnection behaviour and discovery are absent on both. Prefetch ahead of the reading position is not implemented, and neither is the data-saver confirmation on a metered connection. SMB 3 transport encryption is unmet; the app reports what the connection actually negotiated rather than claiming otherwise. **Reachable end to end on both platforms, and streaming: a comic on a share is catalogued from its headers and paged through ranged reads, with nothing written to the device. A PDF and a compressed RAR are fetched first, because `PdfRenderer`, PDFKit and libarchive all want a real file.** Both clients are driven against a real Samba server with `server signing = mandatory`, authenticated. [ADR-0010](../decisions/0010-smb-clients.md) records the two clients |
