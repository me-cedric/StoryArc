...
| `offline-downloads` | partial | Background transfer: nothing continues once the app is suspended, and the spec's other half -- that the app "does not claim a download will finish in the background when the platform will suspend it" -- is met only by silence. The reorder is in the model and tested on both platforms but has no control in the downloads view yet. Automatic cleanup is built and unit-tested on both, and wired into the app on Android only; neither has been seen working on a device. **Built on both: the queue holds for Wi-Fi and for a disk limit and resumes by itself, and finishing a publication removes its download reversibly -- the bytes are moved aside, not deleted, so the ten-second undo is a real undo** |
