...
| `network-share` | partial | iOS has the client but no screens. Reconnection behaviour, discovery, and prefetch ahead of the reading position are absent on both. SMB 3 transport encryption is unmet -- neither client implements it, and the app reports what the connection actually negotiated rather than claiming otherwise. **Android is reachable end to end: a reader enters a host, share and password, is told which of the four failures happened if one does, browses the share's tree to pick a root, and reads a comic streamed from it.** Both clients are driven against a real Samba server with `server signing = mandatory`, authenticated. [ADR-0010](../decisions/0010-smb-clients.md) records the two clients |
