...
| `sources` | partial | Metadata cache. **Connection state is built on both**, and the earlier note here saying it was never probed was wrong: `SourceHealth` on Android and `reach` on iOS have been asking OPDS, Kavita and SMB and recording the answer since they were written. What was missing was the *retry*: one probe on appearance meets neither "exponential backoff starting at 5 seconds and capping at 5 minutes" nor reconnecting "without user action", so a reader whose Wi-Fi came back while they watched the library saw "Connecting…" until they left the screen and returned. `SourceProbe` now holds the schedule — eight tests a side, no network needed — and both platforms loop on it while anything is away, stopping when the library does. **Registry, credential storage, folder-as-source, OPDS-catalogue-as-source, renaming, Settings › Sources, removal and reordering are built.** |
