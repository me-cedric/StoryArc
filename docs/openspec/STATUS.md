...
| `sources` | partial | Metadata cache; **the asking half** of connection state — per-kind reachability for SMB, OPDS and Kavita. `SourceProbe` decides the two things that can be got wrong without a network: the backoff doubles from five seconds and stops at five minutes, and a 401 or 403 is the one state that asks the reader to act while 404, 500 and a refused connection are all simply unreachable. Eight tests a side. Nothing has ever called `SourceRegistry.marking`, so a remote source still shows the state it was constructed with. **Registry, credential storage, folder-as-source, OPDS-catalogue-as-source, renaming, Settings › Sources, removal and reordering are built.** Reordering was driven on both: a drag on iOS, two buttons on Android, each landing the same order and each written through to disk on the spot — the order decides precedence, because the scan walks the registry and the first find of an identity wins |
