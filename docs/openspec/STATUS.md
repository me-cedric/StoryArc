...
| `network-share` | partial | Reconnection behaviour and discovery are absent on both. Streaming is not wired: opening a publication fetches the whole file first, although the ranged-read pieces ([ADR-0008](../decisions/0008-ranged-reads-and-own-zip-reader.md)) and an SMB source that supplies them both exist -- what is missing is a reader that takes a source rather than a path. SMB 3 transport encryption is unmet; the app reports what the connection actually negotiated rather than claiming otherwise. **Reachable end to end on both platforms: a reader enters a host, share and password, is told which of the four failures happened if one does, browses the share's tree to pick a root, and reads a comic from it.** Both clients are driven against a real Samba server with `server signing = mandatory`, authenticated. [ADR-0010](../decisions/0010-smb-clients.md) records the two clients |
