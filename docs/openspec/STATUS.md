...
| `sources` | partial | Metadata cache; connection state is never probed for a remote source. **Registry, credential storage, folder-as-source, OPDS-catalogue-as-source, renaming, Settings › Sources, removal and reordering are built.** Reordering was driven on both: a drag on iOS, two buttons on Android, each landing the same order and each written through to disk on the spot — the order decides precedence, because the scan walks the registry and the first find of an identity wins |
