...
| `sources` | partial | The catalogue half of the metadata cache: a remote source's titles and structure are still re-fetched rather than read from disk, so "the library opens instantly" and "a single unobtrusive indicator states that content is cached and when it was last refreshed" are unmet. **Covers are cached on disk now**, keyed by publication *and* pixel size because "display resolution" is a property of the layout as well as the device — five tests a side, instrumented on Android where a `Bitmap` needs one. They live in the caches directory, which is what makes them "evictable under storage pressure independently of downloaded publications": the system reclaims it, and the Privacy screen already counts and clears it under Cache while downloads sit elsewhere. **Connection state is built on both**, and an earlier note here saying it was never probed was wrong — `SourceHealth` and `reach` have been asking since they were written. What was missing was the retry, and both platforms now loop on `SourceProbe`'s backoff while anything is away. **Registry, credential storage, folder-as-source, OPDS-catalogue-as-source, renaming, Settings › Sources, removal and reordering are built.** |
