...
| `sources` | partial | One clause: the catalogue itself is not cached, so a remote source's titles are re-fetched rather than read from disk and neither "the library opens instantly" nor the "cached, last refreshed at…" indicator is met. **The rest of the metadata cache is built.** Covers are on disk, keyed by publication *and* pixel size, in the caches directory — which is what makes them "evictable independently of downloaded publications" without any code for it. A refresh now updates incrementally on both: Android used to empty the shelf and rebuild it, so a pull-to-refresh blanked the library and re-decoded every cover. And a publication a walk no longer finds is removed while its reading progress stays, because progress lives in its own store keyed by identity — scoped to the folder that was walked, so one source's scan cannot evict another's titles. **Connection state is built on both**, with the retry loop on `SourceProbe`'s backoff. **Registry, credential storage, folder-as-source, OPDS-catalogue-as-source, renaming, Settings › Sources, removal and reordering are built.** |
