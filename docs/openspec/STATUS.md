...
| `library-browsing` | partial | Recent searches; 7 of 11 filters; date-added and file-size sorting. **Unified library, search grouping by match kind, and per-scope layout are built on both platforms.** The shelf now spans sources rather than folders: finished downloads join it, each attributed to the source its record names, so a chapter fetched from Kavita is in the library instead of being reachable only by browsing back to the server. A scope selector narrows the whole view — the shelf, the search, the filters and the continue row — to one source; it persists until changed, and a scope naming a source that has since been removed opens the whole library rather than an empty one. A publication carries its source's name only where more than one source is configured. Search results are grouped as series, titles, people and tags, the headings ordered by the best match rather than by a fixed list, and each publication appears once under the reason it ranked where it did. Grid or list is remembered per scope, falling back to the choice a reader made before the layout was per scope. The grouping, the scoping and the attribution are asserted against the same table in both suites; the download attribution is asserted in both persistence suites. **None of it has been seen running.** No simulator and no emulator was available in this environment, so the scope menu, the group headings, the source line under a cover and the "search all sources" offer have been read rather than watched — and the Android per-scope layout is covered only by an instrumented test that was never run |
