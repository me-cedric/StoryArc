...
| `network-share` | partial | Discovery, prefetch ahead of the reading position, and the data-saver confirmation on a metered connection are absent. SMB 3 transport encryption is unmet; the app reports what the connection actually negotiated rather than claiming otherwise. **Everything else is reachable end to end on both platforms: add a share and be told which of the four failures happened, browse its tree to pick a root, read a comic streamed from it with nothing written to the device, and keep reading across a dropped connection -- pages already read stay, a notice appears after two seconds, an offer to download or leave after sixty, and the session re-establishes itself on the next read.** Driven against a real Samba server with `server signing = mandatory`, authenticated. [ADR-0010](../decisions/0010-smb-clients.md) records the two clients |
