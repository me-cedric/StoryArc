...
| `offline-downloads` | partial | Background transfer: nothing continues once the app is suspended, and the spec's other half -- that the app "does not claim a download will finish in the background when the platform will suspend it" -- is met only by silence. Automatic cleanup is built, unit-tested and wired on both platforms, but has not been seen working on a device. **Everything else is built on both: the queue holds for Wi-Fi and for a disk limit and resumes by itself, queued downloads reorder from the downloads view, and finishing a publication removes its download reversibly -- the bytes are moved aside, not deleted, so the ten-second undo is a real undo** |
