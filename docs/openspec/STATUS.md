...
platforms. A capability is **built** when every requirement has code on both platforms,
...
| `local-library` | partial | Imported copies (whole requirement); watched changes (whole requirement); resumable scan |
...
| `localization` | partial | Language override; plurals on both platforms; locale-correct byte formatting on iOS; no pseudo-locale test; no CI gate on a missing key for iOS |
...
| Check | Covers |
| --- | --- |
| `pnpm check` | specs, tokens, generated notices, fixtures, both apps' lint and unit tests |
...
Nothing checks a screenshot against a reference, nothing tests a pseudo-locale, and
nothing gates a missing iOS string. `native-experience` asks for the first of those by
name.
