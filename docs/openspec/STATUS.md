...
| `offline-downloads` | partial | The queue: no pause, resume, reorder, concurrency bound, background transfer or Wi-Fi-only policy; no storage limit; no automatic cleanup; the storage view does not break down by source beyond naming one; nothing downloads a whole series. **A publication fetched from a catalogue is a recorded download in a backup-excluded directory, verified by indexing, not re-fetched once present, and listed with its size in Settings › Downloads and storage, where it can be removed** |
