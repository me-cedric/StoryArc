...
| `network-share` | partial | Everything a reader can see: no add-a-share sheet, no directory browsing, no listing into the library, no reconnection behaviour, no discovery. **The Android client is built and driven against a real SMB2 server: it connects, reports the dialect it negotiated, lists a folder, and reads at an offset -- the third `RandomAccessSource` [ADR-0008](../decisions/0008-ranged-reads-and-own-zip-reader.md) planned for.** iOS has no client yet; [ADR-0010](../decisions/0010-smb-clients.md) names the one to use |
