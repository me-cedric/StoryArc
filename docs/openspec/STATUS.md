...
| `network-share` | partial | Everything a reader can see: no add-a-share sheet, no directory browsing, no listing into the library, no reconnection behaviour, no discovery. SMB 3 transport encryption is unmet -- neither client implements it, and the app reports what the connection actually negotiated rather than claiming otherwise. **Both clients are built and driven against a real Samba server with `server signing = mandatory`, authenticated: they connect, list a folder, read at an offset -- the third `RandomAccessSource` [ADR-0008](../decisions/0008-ranged-reads-and-own-zip-reader.md) planned for -- and reject a wrong password as such.** [ADR-0010](../decisions/0010-smb-clients.md) records the two clients and where they differ |
