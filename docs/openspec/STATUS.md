...
| `comic-reader` | partial | Border crop, which is the one part of image adjustments still missing -- **brightness, contrast, inversion and greyscale are built on both**, live and per series; sharpness is on iOS and on Android from API 33, where a runtime shader exists. Both were driven end to end, on a simulator and on an emulator. chapter navigation; orientation lock; slider thumbnails; spread pairing; scroll separator; direction override; memory-pressure prefetch shrink |
