...
| `kavita-server` | partial | Android entirely; metadata, server collections and reading lists, progress synchronisation, the continue-point action, covers. **The client is built: authentication with token renewal, version check, extracting a base and key from a pasted OPDS URL, and reading libraries, series, volumes, chapters and server search — reachable on iOS: paste a server's OPDS URL, browse its libraries and series, and read a chapter.** Built against Kavita's documented API and the mock in `scripts/kavita-server.mjs`, never against a live server |
