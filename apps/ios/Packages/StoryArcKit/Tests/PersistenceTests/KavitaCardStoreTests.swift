...
///
/// Android's `KavitaCardStoreTest` makes the same six claims in the same order.
struct KavitaCardStoreTests {
    ...
    func roundTrip() throws {
        ...
        #expect(read.ageRating == 10)
        #expect(read.publicationStatus == 2)
    }
    ...
    func olderCardStillReads() throws {
        ...
        // The values those defaults carry are the claim: zero is Kavita's `Unknown` rating,
        // which draws no line, and -1 is outside Kavita's status table. Zero would have been
        // `OnGoing`, so a card that fell back to it would tell a reader the series is
