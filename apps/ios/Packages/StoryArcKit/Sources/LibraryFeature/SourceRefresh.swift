...
enum LibraryNotice: Equatable {
    ...
    ///    *Nothing to say*. That is a statement about *timing* only: a source that was asked
    ///    and never answered is named at the foot of the shelf instead, by
    ///    ``sourcesNeverReached(in:)`` and ``NeverReachedNotice``, because `library-browsing`
    ///    asks the library to name it rather than to stay quiet about it.
    static func of(
