...
///
/// Every sentence here resolves through `App/Resources/Localizable.xcstrings`, under the
/// `open.in.*` keys Android's `RefusedFileDialog` already draws as `R.string.open_in_*`.
struct RefusedFile: Identifiable, Sendable {
    ...
    /// would promise CB7.
    ///
    /// Data rather than a sentence, which is why it stays a literal while the words around
    /// it are keys: a format name is the same in all four languages, and a catalogue entry
    /// per language would be four places to add the next format to.
    private static let supported = "CBZ, CBR, CBT, EPUB, PDF, M4B"
    ...
    var message: Text {
        ...
        return Text("open.in.unreadable \(name) \(Self.supported)")
    }
}
...
extension View {
    ...
    func refusing(_ file: Binding<RefusedFile?>) -> some View {
        alert(
            Text("open.in.refused.title"),
            isPresented: Binding(get: { file.wrappedValue != nil }, set: { if !$0 { file.wrappedValue = nil } })
        ) {
            Button(role: .cancel) { file.wrappedValue = nil } label: { Text("open.in.dismiss") }
        } message: {
            if let refused = file.wrappedValue { refused.message }
        }
    }
}
