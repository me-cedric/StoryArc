...
class SkippedNoticeTest {
    ...
    /**
     * What those two cases say on screen, in this test's own locale.
     *
     * The entry carries the case and `values/strings.xml` carries the words, so what a node
     * holds is the English value. The other three languages are `SkipReasonWordsTest`'s.
     */
    private val sevenZipWords = "CB7 is not a format StoryArc reads"
    private val protectedWords = "the archive is password protected"

    private val named = "“refused.cb7” couldn’t be opened"

    private fun banner(reason: SkipReason?, onDismiss: () -> Unit = {}) {
        compose.setContent {
    ...
    fun `the notice outlives the six seconds the toast had`() {
        ...
        compose.onNodeWithText(named, substring = true).assertIsDisplayed()
        compose.onNodeWithText(sevenZipWords, substring = true).assertIsDisplayed()
    }
    ...
    fun `the notice is one stop for a screen reader, and the way to the list is a named control`() {
        ...
        compose.onNode(hasText(named, substring = true) and hasText(sevenZipWords, substring = true))
            .assertIsDisplayed()
        // And exactly one, which is what the merge is for: unmerged there are two.
        compose.onAllNodesWithText(sevenZipWords, substring = true).assertCountEquals(1)
    ...
    fun `several state the count and no reason, because the reasons are in the list`() {
        ...
        compose.onAllNodesWithText(sevenZipWords, substring = true).assertCountEquals(0)
        compose.onNodeWithText("What couldn’t be opened").assertIsDisplayed()
    }
    ...
    fun `the list keeps every reason apart`() {
        ...
        compose.onNode(
            hasText(sevenZip.name, substring = true) and hasText(sevenZipWords, substring = true),
        ).assertIsDisplayed()
        compose.onNode(
            hasText(protected.name, substring = true) and
                hasText(protectedWords, substring = true),
        ).assertIsDisplayed()
    }
}
